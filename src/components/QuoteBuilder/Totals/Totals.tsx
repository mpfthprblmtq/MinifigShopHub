...
const Totals = forwardRef(({ storeMode, handleTotalAdjustmentChange }: TotalsSectionParams, ref) => {
  ...
  const calculateValues = (items: Item[]) => {
    if (items.length > 0) {
      // calculate values
      const calculatedBaseValue: number = Math.round(items
        .filter(item => !item.sources.includes(Source.CUSTOM))
      ...
      const adjustmentSet = new Set(items.filter(item => !item.sources.includes(Source.CUSTOM)).map(item => item.valueAdjustment));
  ...
  useEffect(() => {
    if (quote.items.length > 0) {
      const adjustmentSet = new Set(quote.items
        .filter(item => !item.sources.includes(Source.CUSTOM)).map(item => item.valueAdjustment));
      if (adjustmentSet.size > 1) {
        setSliderDisabled(true);
      } else {
        setSliderDisabled(false);
                    ...
                    <Tooltip title={quote.items.filter(item => item.sources.includes(Source.CUSTOM)).length > 0 ?
                      ...
                      <div style={{ width: "140px", minWidth: "140px", maxWidth: "150px" }}>
                        <CurrencyTextInput
                          ...
                          readonly
                          endAdornment={
                            quote.items.filter(item => item.sources.includes(Source.CUSTOM)).length > 0 ?
                              <Info sx={{ color: "gray" }} /> : <></>}
                        />
                      </div>
                    </Tooltip>
